        ...
        private void UpdateState()
        {
            dataTimer = new Timer(new TimerCallback(async (s) =>
            {
                ...
                else
                {
                    ...
                    {
                        case 400:
                            await JS.InvokeVoidAsync("alertMessage", $"{localizer[StringDescriptionModel.account_alert_account_alreadyexist]}");
                            break;
                        case 401:
                            await JS.InvokeVoidAsync("alertMessage", $"{localizer[StringDescriptionModel.httpcode_401]}");
                            break;
                        case 403:
                            await JS.InvokeVoidAsync("alertMessage", $"{localizer[StringDescriptionModel.httpcode_403]}");
                            break;
                        case 404:
                            await JS.InvokeVoidAsync("alertMessage", $"{localizer[StringDescriptionModel.httpcode_404]}");
                            break;
                        default:
                            await JS.InvokeVoidAsync("alertMessage", $"{localizer[StringDescriptionModel.httpcode_urlerr]}");
                            break;
                    }
                }
